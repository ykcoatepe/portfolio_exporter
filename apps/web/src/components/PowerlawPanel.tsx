...
import type { PowerlawSignalsHelp, PowerlawSnapshot } from "../lib/types";
...
type MetricCardProps = {
  ...
  detail?: string | null;
  helpKey?: string;
  help?: PowerlawSignalsHelp;
  helpLoading?: boolean;
  helpError?: Error | null;
};
...
const HelpTooltip = ({ ariaLabel, widthClass = "w-[260px]", children }: HelpTooltipProps) => (
  <div className="relative group">
    ...
    <div
      role="tooltip"
      className={clsx(
        "absolute left-0 top-6 z-20 max-w-[75vw] rounded-2xl border border-slate-800/70 bg-slate-950/95 p-3 text-xs text-slate-200 opacity-0 shadow-xl transition",
        widthClass,
...
const MetricCard = ({
  ...
  const term = helpKey ? help?.terms?.find((entry) => entry.key === helpKey) : null;
  return (
    <div className="rounded-2xl border border-slate-900/60 bg-slate-950/40 p-4">
      <div className="text-xs uppercase tracking-wide text-slate-500">
        <div className="flex items-center gap-1">
          <span>{label}</span>
          {helpKey ? (
            <HelpTooltip ariaLabel={`${label} help`}>
              {helpLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  {term.body ? <p className="text-slate-200">{term.body}</p> : null}
                  {term.bullets?.length ? (
...
export default function PowerlawPanel({ powerlaw, onRefresh }: PowerlawPanelProps): JSX.Element {
  ...
  const { data: help, isLoading: helpLoading, error: helpError } = usePowerlawSignalsHelp({
    enabled: Boolean(powerlaw),
  });
  ...
  const dataQualityRaw = powerlaw.data_quality ?? "UNKNOWN";
  const dataQualityLabel = dataQualityRaw.trim().toUpperCase() || "UNKNOWN";
  const dataQualityDetails =
    powerlaw.data_quality_detail?.filter(
  ...
  const dataQualityTerm = help?.terms?.find((entry) => entry.key === "data_quality");
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-semibold text-slate-100">Powerlaw Signals</h2>
            <HelpTooltip ariaLabel="Powerlaw signals help" widthClass="w-[340px]">
              {helpLoading ? (
                <p className="text-slate-400">Loading explanation...</p>
              ) : helpError ? (
                <p className="text-rose-300">Help unavailable.</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <HelpTooltip ariaLabel="Powerlaw data quality help">
              {helpLoading ? (
                <p className="text-slate-400">Loading explanation...</p>
              ) : helpError ? (
                <p className="text-rose-300">Help unavailable.</p>
                ...
                <div className="space-y-2">
                  {dataQualityTerm ? (
                    <>
                      <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-300">
                        {dataQualityTerm.title}
                      </div>
                      {dataQualityTerm.body ? (
                        <p className="text-slate-200">{dataQualityTerm.body}</p>
                      ) : null}
                      {dataQualityTerm.bullets?.length ? (
                        <ul className="list-disc space-y-1 pl-4 text-slate-200">
                          {dataQualityTerm.bullets.map((bullet, idx) => (
                            <li key={`data-quality-${idx}`}>{bullet}</li>
                          ))}
                        </ul>
                      ) : null}
                    </>
                  ) : null}
                  {dataQualityDetails.length ? (
                    <div className="space-y-1 border-t border-slate-800/70 pt-2">
                      <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-300">
                        Details
                      </div>
                      <ul className="list-disc space-y-1 pl-4 text-slate-200">
                        {dataQualityDetails.map((detail, idx) => (
                          <li key={`data-quality-detail-${idx}`}>{detail}</li>
                        ))}
                      </ul>
                    </div>
                  ) : (
                    <p className="text-slate-400">
                      {dataQualityLabel === "UNKNOWN"
                        ? "No data quality details available."
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-slate-900/60 bg-slate-950/40 p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">
              <div className="flex items-center gap-1">
                <span>Equity Weights</span>
                <HelpTooltip ariaLabel="Equity weights help">
                  {helpLoading ? (
                    <p className="text-slate-400">Loading explanation...</p>
                  ) : helpError ? (
                    <p className="text-rose-300">Help unavailable.</p>
                    ...
                    (() => {
                      const term = help.terms?.find((entry) => entry.key === "equity_weights");
                      if (!term) {
                      ...
                      return (
                        <div className="space-y-2">
                          ...
                          {term.bullets?.length ? (
                            <ul className="list-disc space-y-1 pl-4 text-slate-200">
                              {term.bullets.map((bullet, idx) => (
                                <li key={`equity-weights-${idx}`}>{bullet}</li>
                              ))}
                            </ul>
                          ) : null}
                        </div>
          ...
          <div className="rounded-2xl border border-slate-900/60 bg-slate-950/40 p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">
              <div className="flex items-center gap-1">
                ...
                <HelpTooltip ariaLabel="Hedge notional help">
                  {helpLoading ? (
                    <p className="text-slate-400">Loading explanation...</p>
                  ) : helpError ? (
                    <p className="text-rose-300">Help unavailable.</p>
                    ...
                    (() => {
                      const term = help.terms?.find((entry) => entry.key === "hedge_notional");
                      if (!term) {
                        return <p className="text-slate-400">No details available.</p>;
                      }
                      return (
                        <div className="space-y-2">
                          ...
                          {term.bullets?.length ? (
                            <ul className="list-disc space-y-1 pl-4 text-slate-200">
                              {term.bullets.map((bullet, idx) => (
                                <li key={`hedge-notional-${idx}`}>{bullet}</li>
                              ))}
                            </ul>
                          ) : null}
                        </div>
