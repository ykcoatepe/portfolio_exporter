  ...
  const baseView = {
    single_stocks: [
      ...
      },
      {
        ...
        previous_close: 378.0,
        updated_at: new Date(now - 55_000).toISOString(),
        greeks: { delta: 30 },
        conId: 1004,
      },
      {
        ...
        updated_at: new Date(now - 70_000).toISOString(),
        greeks: { delta: 20 },
        conId: 1005,
      },
    ],
...
const msbHelpFixture = {
  title: "MSB Signals Guide",
  subtitle: "Sample MSB help content for mocks",
  sections: [{ title: "Overview", bullets: ["Sample MSB help bullet."] }],
  terms: [],
  footnotes: [],
};
...
export const handlers = [
  ...
  http.get("*/stats/current", () => HttpResponse.json(buildStatsResponse())),
  http.get("*/stats", () => HttpResponse.json(buildStatsResponse())),
  http.get("*/session", () => HttpResponse.json(buildSessionFixture())),
  http.get("*/msb/current", () => HttpResponse.json(buildMsbReading())),
  http.get("*/msb/history", () => HttpResponse.json(buildMsbHistory())),
  http.get("*/msb/help", () => HttpResponse.json(msbHelpFixture)),
  http.get("*/msb/status", () => HttpResponse.json(buildMsbStatusFixture())),
  http.post("*/msb/refresh", () => HttpResponse.json({ ok: true, status: "ok" })),
  http.get("*/sse", () =>
    HttpResponse.text("event: ping\ndata: {}\n\n", {
      headers: {
