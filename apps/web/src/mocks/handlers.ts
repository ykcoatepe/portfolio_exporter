...
export const buildOptionsResponse = (
  ...
  const baseLegs: OptionComboLegApi[] = [
    ...
    },
    {
      ...
      label: "AAPL 190C • Sep 20 '24",
      display: {
        leg_label: "AAPL 190C • Sep 20 '24",
        short_ul: "AAPL",
  ...
  const comboGroups = [
    {
      ...
      dte: 32,
      sum_greeks: baseCombos[0].greeks,
      mark_source: "MID" as const,
      stale_seconds: 180,
      label: baseCombos[0].label,
      display: baseCombos[0].display,
      legs: baseLegs
        .filter((leg) => leg.combo_group_id === "group-iron-condor")
        .map((leg) => ({
    ...
    },
    {
      ...
      group_net_price: -4.4,
      dte: 60,
      sum_greeks: baseCombos[1].greeks,
      mark_source: "LAST" as const,
      stale_seconds: 300,
      label: baseCombos[1].label,
      display: baseCombos[1].display,
      legs: baseLegs
        .filter((leg) => leg.combo_group_id === "group-call-diagonal")
        .map((leg) => ({
          symbol: leg.symbol ?? leg.id ?? `${leg.underlying}-${leg.expiry}-${leg.right}-${leg.strike}`,
          underlying: leg.underlying,
          right: leg.right,
