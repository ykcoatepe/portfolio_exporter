...
def test_option_leg_validation():
    ...
    with pytest.raises(ValueError):
        OptionLeg(
            symbol="", expiry="20250117", right="C", strike=150.0, qty=1, price=2.5
        )
    with pytest.raises(ValueError):
        OptionLeg(
            symbol="AAPL", expiry="20250117", right="C", strike=0.0, qty=1, price=2.5
        )
    with pytest.raises(ValueError):
        OptionLeg(
            symbol="AAPL", expiry="20250117", right="C", strike=150.0, qty=0, price=2.5
        )
...
def test_position_validation():
    ...
    with pytest.raises(ValueError):
        Position(
            uid="", symbol="MSFT", sleeve="core", kind="equity", qty=10, mark=320.0
        )
    with pytest.raises(ValueError):
