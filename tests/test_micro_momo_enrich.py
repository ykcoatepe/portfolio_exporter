...
def test_enrich_inplace_with_mocks(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ib_provider, "get_quote", lambda s, c: {"last": 10.0, "prev_close": 9.0}
    )
    # minute bars (10 bars)
    bars = [
        {
    ...
    monkeypatch.setattr(
        ib_provider, "get_intraday_bars", lambda s, c, minutes=60, prepost=True: bars
    )
    ...
    monkeypatch.setattr(
        ib_provider,
        "get_shortable",
        lambda s, c: {"available": 100000, "fee_rate": 8.5},
    )
