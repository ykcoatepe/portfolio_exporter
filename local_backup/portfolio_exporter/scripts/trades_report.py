...
def _compute_streaming_effect(df: pd.DataFrame, prev_positions: pd.DataFrame | None) -> pd.Series:
    ...
    d["__k2"] = pd.to_numeric(d.get("strike"), errors="coerce").apply(
        lambda x: float(f"{x:.2f}") if pd.notna(x) else np.nan
    )
    d["__ts"] = pd.to_datetime(d.get("datetime"), errors="coerce")
    d["__key_valid"] = (
        d["__sym"].ne("") & d["__right"].isin(["C", "P"]) & d["__exp"].ne("NaT") & d["__k2"].notna()
    )
...
def _enrich_combo_strikes_fallback(
    ...
    fmt = lambda x: (f"{float(x):.1f}".rstrip("0").rstrip("."))
...
def _apply_filters(
    ...
    if struct_in:
        cl_f = cl_f[
            cl_f.get("structure").astype(str).str.lower().apply(lambda s: any(v in s for v in struct_in))
        ]

    struct_out = [
        s.strip().lower() for s in _parse_list_arg(getattr(args, "structure_out", None)) if s.strip()
    ]
    if struct_out:
        cl_f = cl_f[
            ~cl_f.get("structure").astype(str).str.lower().apply(lambda s: any(v in s for v in struct_out))
        ]
