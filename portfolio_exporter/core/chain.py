...
def _infer_width_from_legs(legs: list[dict]) -> float | None:
    """
    ...
    if n == 4 and set(rights) == {"C", "P"}:
        call_strikes = [s for s, r in zip(strikes, rights, strict=True) if r == "C"]
        put_strikes = [s for s, r in zip(strikes, rights, strict=True) if r == "P"]
...
def _infer_credit_debit(legs: list[dict]) -> str | None:
    """
...
def _infer_type_and_width(
    ...
    for leg in legs:
        ...
        right = (leg.get("right") or "").upper() if leg.get("right") else None
    ...
    n = len(clean)
    strikes = [leg_row["strike"] for leg_row in clean if leg_row["strike"] is not None]
    rights = [leg_row["right"] for leg_row in clean if leg_row["right"] is not None]
    expiries = [leg_row["expiry"] for leg_row in clean if leg_row["expiry"] is not None]
    ...
    if (
        n == 2
        and same_right
        and len({leg_row["strike"] for leg_row in clean}) == 1
